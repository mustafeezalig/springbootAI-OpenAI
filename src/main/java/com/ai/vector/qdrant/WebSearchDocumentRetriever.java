...
public class WebSearchDocumentRetriever implements DocumentRetriever {
	...
	public WebSearchDocumentRetriever(RestClient.Builder clientBuilder, int resultLimit) {
		...
		this.restClient = clientBuilder.baseUrl(TAVILY_BASE_URL)
	...
	public List<Document> retrieve(Query query) {
		...
		TavilyResponsePayload response = restClient.post().body(new TavilyRequestPayload(q, "advanced", resultLimit))
				.retrieve().body(TavilyResponsePayload.class);
